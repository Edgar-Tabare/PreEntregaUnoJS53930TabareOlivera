...
while (true) {
  ...
  let conAmortizacion = confirm(
    "La cuota social incluye amortización? (Aceptar para sí, Cancelar para no)"
  );
  let fechaVencimientoInput = prompt(
    'Ingrese la fecha de vencimiento de la cuota en formato "YYYY-MM-DD": '
  );
  let fechaVencimiento = new Date(fechaVencimientoInput);

  let fechaPago = prompt(
    'Ingrese la fecha de pago de la cuota en formato "YYYY-MM-DD": '
  );
  let fechaPagoCuotaParseada = new Date(fechaPago);

  let cuotaTotal = calcularCuotaTotal(
    conAmortizacion,
    fechaVencimiento,
    fechaPagoCuotaParseada
  );

  alert(
    `Detalle de deuda para el socio ${numeroSocio} - ${nombre}:\nCuota social: ${cuotaTotal.toFixed(
      2
